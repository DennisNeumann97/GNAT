...
class multipoles:
    ...
    def return_multipole_prefactors_for_gplus(
        ...
        return [alpha_2gplus, alpha_4gplus]
    ...
    # TODO: ++ IS PROBABLY WRONG HERE
    def return_multipole_prefactors_for_plusplus(
        self,
    ) -> list[float]:
        """
        Returns the geometric prefactors for the single ++ multipole, which is unity because shape-shape has no RSDs
    ...
    def return_all_multipoles_from_power_spectrum(
        ...
        Args:
            power_spectrum_spline (sp.interpolate.interp1d): power spectrum spline obtained from the powerSpectrum class
            probe_type (str, optional): type of probe, either 'g+' or 'gg'. Defaults to 'g+'. Also determines the spin of the field (0 for gg, 2 for g+).
        ...
        if probe_type == 'g+':
            ...
            ell_list = [0,2,4]
            alpha_ells = self.return_multipole_prefactors_for_plusplus()
        else:
